...
class SEEDERLossTopDist(nn.Module):
    def __init__(self, instanceQ, model, teacher_temp=0.05, student_temp=0.07, device=None, number_of_terms=2, top_distribution=20):
        """
        ...
        """
        super(SEEDERLossTopDist, self).__init__()
        ...
        self.top_distribution = top_distribution
    ...
    def forward(self, 
        ...
        # Find top k teacher distribution
        percent = round((self.top_distribution / 100) * logit_en_t.shape[1])
        logit_en_t, index = torch.topk(logit_en_t, percent) 
        logit_ne_s = [logit_ne_s[i].index_select(0, index[i]) for i in range(index.shape[0])]
        logit_en_s = [logit_en_s[i].index_select(0, index[i]) for i in range(index.shape[0])]
        logit_ne_s = torch.stack(logit_ne_s)
        logit_en_s = torch.stack(logit_en_s)
