...
class SEEDERLoss(nn.Module):
    def __init__(self, instanceQ, model, teacher_temp=0.01, student_temp=0.2, device=None, number_of_terms=3):
        ...
        """
        super(SEEDERLoss, self).__init__()
    ...
    def forward(self, 
        ...
        loss_s_s_ne = -torch.mul(F.softmax(en_S_Dist, dim=1), F.log_softmax(ne_S_Dist, dim=1)).sum() / batch_size

        # print(f"Loss1:{loss_t_s_en:.4f} Loss2:{loss_t_s_ne:.4f} Loss3:{loss_s_s_ne:.4f}")

        if self.terms == 3:
            seeder_loss = loss_t_s_en + loss_t_s_ne + loss_s_s_ne
        elif self.terms == 2:
