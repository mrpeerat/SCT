...
class BARLOWLoss_Distill(nn.Module):
    def __init__(self, model, embed_size, batch_size, lambda_weight=0.0005, w1=1, w2=1, w3=1, device=None, projector='4096-4096-4096'):
        ...
        """
        super(BARLOWLoss_Distill, self).__init__()
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], teacher_reps: Tensor):
        ...
        c_q_q = self.bn(non_en_question).T @ self.bn(teacher_reps) # question-to-question
        c_d_q = self.bn(en_doc).T @ self.bn(teacher_reps) # doc-to-question
