...
class SEEDERLoss_v4(nn.Module):
    def __init__(self, instanceQ, model, teacher_temp=0.05, student_temp=0.07, device=None, number_of_terms=2, alpha=0):
        """
        ...
        """
        super(SEEDERLoss_v4, self).__init__()
    ...
    def forward(self, 
        ...
        loss_t_s_ne = max((-torch.mul(en_T_Dist, F.log_softmax(ne_S_Dist, dim=1)).sum() / batch_size)-self.alpha,0)
