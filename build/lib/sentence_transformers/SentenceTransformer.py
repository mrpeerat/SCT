...
class SentenceTransformer(nn.Sequential):    
    ...
    #             raise Exception(len(example.label),len(example.label[0]),len(example.label[1]),len(example.label[0][0]))
                rep_sent_t_1.append(example.label[0])
                rep_sent_t_2.append(example.label[1])

    #         raise Exception(len(rep_sent_t),len(rep_sent_t[0]),len(rep_sent_t[0][0]),rep_sent_t[0][0])
    ...
    def fit(self,
        ...
        if str(type(loss_model)) == "<class 'sentence_transformers.losses.ARL_DIS_Loss.ARL_DIS_Loss'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.ARL_DIS_Loss_v2.ARL_DIS_Loss_v2'>":
            print('Momentum initialize')
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                ...
                for train_idx in range(num_train_objectives):
                    ...
                    if str(type(loss_model)) == "<class 'sentence_transformers.losses.DISCOLoss.DISCOLoss'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.SEEDERLoss_v2.SEEDERLoss_v2'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.SEEDERLoss_v3.SEEDERLoss_v3'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.SEEDERLoss_v4.SEEDERLoss_v4'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.SEEDERLossTopDist.SEEDERLossTopDist'>":
                        sentence_en_features, sentence_ne_features, rep_sent_en_t = data
#                         raise Exception(f"Size:{sentence_en_features},{len(sentence_en_features['input_ids'])},{len(sentence_ne_features)}")
                    elif str(type(loss_model)) == "<class 'sentence_transformers.losses.ARLSymLoss_v2_distillation.ARLSymLoss_v2_distillation'>":
                    ...
                    if use_amp:
                        with autocast():
                            if str(type(loss_model)) == "<class 'sentence_transformers.losses.DISCOLoss.DISCOLoss'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.SEEDERLoss_v2.SEEDERLoss_v2'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.SEEDERLoss_v4.SEEDERLoss_v4'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.SEEDERLossTopDist.SEEDERLossTopDist'>":
                        ...
                        loss_value = loss_model(sentence_ne_features,
                                                sentence_en_features,
                                                rep_sent_en_t)
                        loss_value.backward()
                        torch.nn.utils.clip_grad_norm_(loss_model.parameters(), max_grad_norm)
                        optimizer.step()
                    if str(type(loss_model)) == "<class 'sentence_transformers.losses.BYOLoss.BYOLoss'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.ARL_Loss.ARL_Loss'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.ARL_DIS_Loss.ARL_DIS_Loss'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.ARL_DIS_Loss_v2.ARL_DIS_Loss_v2'>" or str(type(loss_model)) == "<class 'sentence_transformers.losses.ARLSymLoss.ARLSymLoss'>":
                        if check_momentum == 0:
