...
class ARLSymLoss_v2_debug_CE(nn.Module):
    def __init__(self, instanceQ_A, instanceQ_B, teacher_temp, model, sentence_embedding_dimension, student_temp=0.05, device=None,path_model='X'):
        ...
        """
        super(ARLSymLoss_v2_debug_CE, self).__init__()
...
#         ARL_loss_score_B = -torch.mul(momentum_Dist_B.detach(), F.log_softmax(sentence_Dist_B, dim=1)).sum() / batch_size
#         ARL_loss_score_A = -torch.mul(momentum_Dist_A.detach(), F.log_softmax(sentence_Dist_A, dim=1)).sum() / batch_size
