...
class ARL_Loss_v2(nn.Module):
    def __init__(self, instanceQ, teacher_temp, model, sentence_embedding_dimension, moving_average_decay=0.999, student_temp=0.05, device=None):
        ...
        """
        super(ARL_Loss_v2, self).__init__()
    ...
    def forward(self, 
        ...
        with torch.no_grad():
            rep_sent_momentum = self.model(sent_B)['sentence_embedding']
        ...
        # bootstrap_loss = 2 - 2 * (rep_sent_A * rep_sent_momentum.detach()).sum(dim=-1) 
        
        # loss = (
        #     self.lambda_ * ARL_loss_score
        #     + self.beta * bootstrap_loss
        # )
