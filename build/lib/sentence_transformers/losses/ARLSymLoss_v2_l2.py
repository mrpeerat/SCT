...
class ARLSymLoss_v2_l2(nn.Module):
    def __init__(self, instanceQ_A, instanceQ_B, batch_size, teacher_temp, model, sentence_embedding_dimension, beta_val, lambda_val, student_temp=0.05, device=None,path_model='X', feature_mode='rep'):
        """
        ...
        """
        super(ARLSymLoss_v2_l2, self).__init__()
        ...
        self.beta_ = beta_val
        self.loss_fct_distance = nn.MSELoss()
    ...
    def forward(self, 
        ...
        if self.feature_mode == 'rep':
            distance_loss = self.loss_fct_distance(rep_sent_A, rep_sent_B)
        elif self.feature_mode == 'dis':
            distance_loss = self.loss_fct_distance(sentence_Dist_A, sentence_Dist_B)
        else:
            raise Exception(f'Error mode:{self.feature_mode}')
        
        Final_loss = (self.beta_*ARL_loss_score)+(self.lambda_*distance_loss)
